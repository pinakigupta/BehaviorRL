...
class AbstractLane(object):
    ...
    def on_lane(self, position, longitudinal=None, lateral=None):
        ...
        is_on = np.abs(lateral) <= self.width_at(longitudinal) / 2 and \
                -Vehicle.DEFAULT_CONFIG["DEFAULT_LENGTH"] <= longitudinal < self.length + Vehicle.DEFAULT_CONFIG["DEFAULT_LENGTH"]
        return is_on
    ...
    def is_reachable_from(self, position):
        ...
        if self.forbidden:
            return False
        longitudinal, lateral = self.local_coordinates(position)
        is_close = np.abs(lateral) <= 2 * self.width_at(longitudinal) and 0 <= longitudinal < self.length + \
                    Vehicle.DEFAULT_CONFIG["DEFAULT_LENGTH"]
        return is_close

    def after_end(self, position, longitudinal=None, lateral=None):
        if not longitudinal:
            longitudinal, _ = self.local_coordinates(position)
        return longitudinal > self.length - Vehicle.DEFAULT_CONFIG["DEFAULT_LENGTH"] / 2
